...
private const val RECOGNITION_INTERVAL_MS = 10000L // 10 seconds between recognition attempts
...
class AutoRecognitionService : Service() {
    ...
    private suspend fun handleRecognitionResult(result: RecognitionResult) {
        when (result) {
            is RecognitionResult.Success -> {
                ...
                // Only notify if it's a different song
                if (trackId != lastRecognizedTrackId) {
                    lastRecognizedTrackId = trackId
                    showSongNotification(result.track)
                    Log.d(TAG, "New song recognized: ${result.track.title}")
                } else {
                    Log.d(TAG, "Same song detected, skipping notification: ${result.track.title}")
                }
            }
            ...
            is RecognitionResult.ScheduledOffline -> {
                Log.d(TAG, "Recognition scheduled offline")
            }
        }
    }
    ...
    private suspend fun showSongNotification(track: Track) {
        ...
        // Add album artwork as large icon if available
        ...
        // Show the individual notification first
        notificationManager.notify(notificationId, notificationBuilder.build())

        // Show/update the summary notification (group header)
        val summaryNotification = NotificationCompat.Builder(this, NOTIFICATION_CHANNEL_ID_RESULT)
            ...
            .setContentTitle("Audile")
            .setContentText("$recognizedSongsCount songs recognized")
            .setWhen(timestamp)
            .setShowWhen(false)
            .setGroup(NOTIFICATION_GROUP_KEY)
            ...
            .setStyle(NotificationCompat.InboxStyle()
                .setSummaryText("$recognizedSongsCount songs recognized"))
            .build()
    ...
    private fun stopAutoRecognition() {
        ...
        // Update preference to reflect stopped state
        serviceScope.launch {
    ...
    private fun createNotificationChannel() {
        ...
        val resultChannel = NotificationChannel(
            ...
            description = "Shows recognized songs from auto-recognition"
            setShowBadge(true)
        }
    ...
    private fun createNotification(): Notification {
        ...
        // Delete intent - triggered when notification is swiped away
        ...
        return NotificationCompat.Builder(this, NOTIFICATION_CHANNEL_ID)
            ...
            .setDeleteIntent(deletePendingIntent) // Stop service when swiped
            .setPriority(NotificationCompat.PRIORITY_LOW)
            .addAction(
                android.R.drawable.ic_menu_close_clear_cancel,
                getString(StringsR.string.stop),
                stopPendingIntent
            )
            .build()
    }
    ...
    companion object {
        ...
        fun isRunning(context: Context): Boolean {
            ...
            for (service in manager.getRunningServices(Int.MAX_VALUE)) {
                if (AutoRecognitionService::class.java.name == service.service.className) {
                    return true
                }
            }
            return false
